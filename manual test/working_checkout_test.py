...
def test_checkout_flow():
    ...
    log_info("Getting available products...")
    products = get_products()
    if not products:
        log_error("No products found. Please ensure products are available in the database.")
        return False
    
    if len(products['results']) < 2:
        log_warning("Less than 2 products available. Using what's available.")
    
    # Step 3: Add products to cart
    product_id1 = products['results'][0]['id']
    log_info(f"Adding product {product_id1} to cart...")
    add_to_cart(product_id=product_id1, quantity=2)
    
    if len(products['results']) > 1:
        product_id2 = products['results'][1]['id']
        log_info(f"Adding product {product_id2} to cart...")
